...
Tensors only accept rectangular shapes (think matrices).
Since the inner lists have different lengths, this array is not yet rectangular, so we need to pad the shorter ones before converting it to a tensor.
